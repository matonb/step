...
module suite it needs docker, so it does not run on every pull request: both
suites run on pull requests touching `plugins/`, `roles/` or
`tests/integration/`, and can be started from the Actions tab for anything
else. Neither gates a merge, but both gate a release.
